...
// logging
// console.log(JSON.stringify(processInstructions, null, '   '));
// console.log(JSON.stringify(prebuilderOptions, null, '   '));
// console.log(JSON.stringify(rollupArgs, null, '   '));
...
if (processInstructions.args.some(arg => ['-w', '-watch'].includes(arg))) {
    ...
    if (prebuilderOptions.onTheSpot) {
        ...
        This method consisted in using rollup's watch event, to implement automatic resolve/restore
        this has been abandoned because upon watch.onEnd event's execution of restore,
        files are changed causing a resolve/restore loop,
